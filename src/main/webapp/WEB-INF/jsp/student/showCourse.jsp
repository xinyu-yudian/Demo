...
<body>
				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${courseList}" var="item">
								<tr>
									...
									<td>${item.haspeople}</td>
									<td class="nopeople">${item.nopeople}</td>
									<td >${item.score}</td>
									<td>
										<c:if test="${not empty  cda}">
										<button class="btn btn-default btn-xs btn-info selectcource">选课</button>
										</c:if>
										<c:if test="${ empty  cda}">
											<button class="btn btn-default disabled btn-xs btn-info ">暂无操作</button>
										</c:if>
										<!--弹出框-->
									</td>
								</tr>
	...
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">此课程已满，请选择别的课程</div>
	...
	<div class="modal fade" id="myModalcount" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel1">温馨提示：</h4>
				</div>
				<div class="modal-body">您已选择课程，每人只能选择一门课程</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" data-dismiss="modal">关闭</button>
					<%--<button type="button" class="btn btn-primary">提交更改</button>--%>
				</div>
